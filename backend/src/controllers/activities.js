...
activityController.post("activities/upload-xml", auth(["admin"]), async (req, res) => {
  if (req.files && req.files["xml-file"]) {
    ...
    //   console.log("activityUpload", activityUpload);

      const activityUploadAttributes = activityUpload["$"];
      console.log("activityUploadAttributes: ", activityUploadAttributes);
    ...
    //   console.log("activitiesData: ", activitiesData);
      ...
      if (operation === "insert") {
        // Convert the xml objects into model objects and perform the insert operations
        await Promise.all(
          activitiesData.map(async (activityData) => {
            // Convert the xml object into a model object
            // Extract and clean up values from XML
            const name = activityData.name[0].trim();
            const description = activityData.description[0].trim();
            console.log("description is logged haha: ", description)
            const duration = activityData.duration[0].trim()
        ...
        // Convert the xml objects into model objects and perform the update operations
        await Promise.all(
          activitiesData.map(async (activityData) => {
            // Convert the xml object into a model object
            const activityModel = Activities.newActivity(
              ...
              activitiesData.duration.toString()
            );
